...
void main() {
  test(
    'all legacy interactive entries have one closed execution classification',
    () {
      final root =
          jsonDecode(
                File(
      ...
      for (final classification in <String, int>{
        ...
        expect(
          entries.where(
            (entry) => entry['classification'] == classification.key,
          ),
      ...
      expect(
        ...
        isTrue,
        reason:
            'classified legacy scripts must not ship as executable payloads',
      );
      expect(
        entries.where(
          (entry) =>
