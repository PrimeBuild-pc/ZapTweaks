...
$localMouseRecorder = Join-Path $programmiPath "mousemovementrecorder.exe"
$installPath = "$env:SystemDrive\Program Files (x86)\Mouse Movement Recorder\Mouse Movement Recorder.exe"

if (!(Test-Path $localMouseRecorder)) {
Write-Host "Missing local file: $localMouseRecorder" -ForegroundColor Red
Pause
exit
}
...
# use local mouse movement recorder
Copy-Item -Path $localMouseRecorder -Destination $installPath -Force
