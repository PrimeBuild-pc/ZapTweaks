...
| 6 — Windows and diagnostics | In progress | Cleanup preview scanner, deterministic diagnostic-session lifecycle, native SCM inspection of live state/start type/delayed start/account/PID/dependencies, allowlisted exact-snapshot SCM startup mutation without silently stopping a running service, and allowlisted Task Scheduler enable/disable with typed read-back and exact rollback, plus bounded on-demand WPR/ETW capture that always stops and rejects empty traces | ETW/DPC analysis UI, repair and recovery UI |
...
- `flutter test -j 1`: 172 tests passed.
