...
| 5 — Gaming and hardware | In progress | Topology-aware RSS, MSI limit and non-truncating affinity validators; native processor-group and NUMA topology inventory; native PowrProf scheme enumeration, active-scheme detection and AC/DC value I/O; processor boost mode and maximum processor state now route through exact-snapshot PowrProf operations; a real VM round trip changed AC maximum processor state, verified it, and restored AC/DC plus the active scheme | Complete power-plan management and remaining fixed-setting migrations, then SetupAPI/IRQ and NDIS platform implementations |
...
- `flutter test -j 1`: 159 tests passed.
