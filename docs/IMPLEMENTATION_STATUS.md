...
| 5 — Gaming and hardware | In progress | Topology-aware RSS, MSI limit and non-truncating affinity validators; native processor-group and NUMA topology inventory; provider-backed NDIS RSS inspection/mutation with complete tuple snapshots, live topology/range validation and no registry fallback; native PowrProf scheme enumeration, active-scheme detection and AC/DC value I/O; processor boost mode and maximum processor state now route through exact-snapshot PowrProf operations; a real VM round trip changed AC maximum processor state, verified it, and restored AC/DC plus the active scheme | Complete power-plan management and remaining fixed-setting migrations, validate NDIS RSS in a VM, then implement SetupAPI/IRQ/MSI platform mutations |
...
- `flutter test -j 1`: 161 tests passed.
