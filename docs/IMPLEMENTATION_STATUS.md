...
| 5 — Gaming and hardware | In progress | Topology-aware RSS, MSI limit and non-truncating affinity validators; native processor-group and NUMA topology inventory; provider-backed NDIS RSS inspection/mutation with complete tuple snapshots, live topology/range validation and no registry fallback; SetupAPI reads documented PCI Line/MSI/MSI-X support and hardware message maxima, while a typed operation limits changes to present display/network/media devices and snapshots MSI values exactly; native PowrProf scheme enumeration, active-scheme detection and AC/DC value I/O; processor boost mode and maximum processor state now route through exact-snapshot PowrProf operations; a real VM round trip changed AC maximum processor state, verified it, and restored AC/DC plus the active scheme | Complete power-plan management and remaining fixed-setting migrations, validate NDIS RSS in a VM, validate NDIS RSS mutation/rollback and MSI mutation/rollback in a VM, then add interrupt-affinity policy and remaining hardware-specific UI |
...
- `flutter test -j 1`: 164 tests passed.
